...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		if (!array_key_exists('posisi',$this->_var_tmp)){
			$this->_var_tmp['posisi']=TRUE;
		}
		if (!array_key_exists('menukanan',$this->_var_tmp)){
			$this->_var_tmp['menukanan']=FALSE;
		}
		
		// Output template variables to the template
		// doi::dump($this->_title,false,true);
		
		$template['tmp_data']	= $this->_var_tmp;
		...
		if ($this->_layout)
		{
			...
			$this->_body =  self::_load_view('layout_'.$this->_layout, $this->_data, TRUE, self::_find_view_folder());
		}
	...
	public function title()
	{
		...
		if (func_num_args() >= 1)
		{
			...
			// doi::dump($title_segments,false,true);
	...
	public function var_tmp($key,$data){
		$this->_var_tmp[$key]=$data;
		return $this;
	}
